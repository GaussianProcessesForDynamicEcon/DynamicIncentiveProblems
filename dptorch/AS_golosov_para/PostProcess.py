...
def process(m, cfg):
    ...
    plot_sample_01 = (
        torch.rand(
            [n_plot_pts, m.cfg["model"]["params"]["n_agents"]]
        )
    )

    n_agents = m.cfg["model"]["params"]["discrete_state_dim"]
    lower_w = m.cfg["model"]["params"]["lower_w"]
    upper_w = m.cfg["model"]["params"]["upper_w"]
    for indxd in range(n_agents):
...
def compare(m1, m2, cfg):
    ...
    state_sample_01 = (
        torch.rand(
            [n_test_pts, m1.cfg["model"]["params"]["n_agents"]]
        )
    )
    ...
    state_sample = (upper_w - lower_w) * state_sample_01 + lower_w
    ...
    for indxt in range(n_sim_steps):
       ...
       #     current_state[0, -1] = 0.
        ...
        with torch.no_grad(), gpytorch.settings.fast_pred_var():
            ...
            for indxtype in range(n_agents):
