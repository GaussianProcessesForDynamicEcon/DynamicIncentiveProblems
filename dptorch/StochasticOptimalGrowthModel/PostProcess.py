...
def process(m, cfg):
    ...
    for d in range(m.discrete_state_dim):
        ...
        for p in range(m.policy_dim):
            ...
            with torch.no_grad(), gpytorch.settings.fast_pred_var():
                pred = m.likelihood[d][1 + p](m.M[d][1 + p](med_states.to(m.device)))
            ...
            ax.legend([f"P[{p}] Mean", f"P[{p}] Confidence"])
            ax.set_xlabel("State 0")

            plt.savefig(f"P_{d}_{p}_fun.png")
            plt.close()

    # run trajectories
    state_trajectory, control_trajectory, eval_g_trajectory = m.generate_trajectory(1000)
    ...
    pd.DataFrame(torch.cat([torch.unsqueeze(x,dim=0) for x in control_trajectory]).numpy()).to_csv('control_trajectory.csv')
...
def compare(m1, m2, cfg):
    ...
    m1.M[0][0].eval()
    m1.likelihood[0][0].eval()
    m2.M[0][0].eval()
    m2.likelihood[0][0].eval()
    with torch.no_grad(), gpytorch.settings.fast_pred_var():
        p1 = m1.likelihood[0][0](m1.M[0][0](state_sample))
        V_p1 = p1.mean
        p2 = m2.likelihood[0][0](m2.M[0][0](state_sample))
        V_p2 = p2.mean
